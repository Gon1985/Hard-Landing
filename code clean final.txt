...
class HardLandingSimulator:
    def __init__(self, root):
        ...
        self.execute_button.pack(pady=(10, 10))  
        ...
        self.convert_window_open = False  
    ...
    def open_aircraft_window(self):
        ...
        if not self.aircraft_window_open:
            ...
            button_sound = pygame.mixer.Sound("sounds/button-object-id.mp3")  
            
            
            self.aircraft_image_label = tk.Label(aircraft_window)
            self.aircraft_image_label.pack()

            self.aircraft_text_label = tk.Label(aircraft_window, text="", font=("pixelmix", 8), bg="black", fg="#31ff03")  
            ...
            previous_button.config(command=lambda: [button_sound.play(), self.previous_aircraft()])  

            
            next_button = tk.Button(aircraft_window, text="Next", command=self.next_aircraft, font=("Arial", 10), bg="#1e1f1e", fg="white")
            next_button.pack(side=tk.RIGHT, padx=10, pady=10)
            next_button.config(command=lambda: [button_sound.play(), self.next_aircraft()])  
    ...
    def open_timer_window(self):
        ...
        """
        timer_sound = pygame.mixer.Sound("sounds/timer.mp3")  
    ...
    def open_context_menu(self, event):
        ...
        if not isinstance(event.widget, tk.Toplevel):
            ...
            context_menu.add_command(label="Aircraft / Objects ID", command=self.open_aircraft_window)  
            context_menu.add_separator()
            context_menu.add_command(label="Help", command=self.open_help_window)  
    ...
    def open_help_window(self):
        ...
        if not self.help_window_open:
            ...
            help_window.configure(bg='#080808')  
    ...
    def execute_simulation(self):
        ...
        if scenario in self.reference_model_speeds_kmph:  
            acceleration = self.reference_model_speeds_kmph[scenario] / 36  
    ...
    def convert_to_kg_to_ton(self):
        ...
        convert_bg_label.image = convert_bg_image  
